...
const Pagination = ({ previous, next, currentPage, setCurrentPage, count }) => {
  ...
  return (
    <>
      {count > 24 ? (
        <nav
          className="Page navigation example mt-5"
            ...
            {numberOfPages ? (
              <ul className="pagination pg-blue">
                {previous ? (
                  <>
                    <li className="page-item">
                      <a
                        className="btn btn-outline-success btn-sm mb-4 pg-lin btn-s"
                        onClick={() => handelFirstPage()}
                      >
                        <span>&laquo;</span>
                      </a>
                    </li>
                ...
                {pagesToRender.map((pageNumber) => (
                  <li
                    key={pageNumber - 1}
                    className={`page-item ${pageNumber === currentPage ? "page-active" : ""}`}
                  >
                    <a
                      className="btn btn-success btn-sm mb-4 pg-lin btn-s"
                      onClick={() => handelPageNumber(pageNumber - 1)}
                    >
                      {pageNumber}
                    </a>
                  </li>
